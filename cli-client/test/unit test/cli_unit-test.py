...
class TestCLI(unittest.TestCase):
    ...
    @patch("cli.get_db_connection")
    def test_usermod_create_user(self, mock_db, mock_admin):
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = None  # User does not exist
        mock_db.return_value.cursor.return_value = mock_cursor

        args = MagicMock(username="testuser", passw="testpassword")
        cli.usermod(args)

        mock_cursor.execute.assert_any_call(
            "INSERT INTO user (username, password, user_role) VALUES (%s, %s, 'operator')",
    ...
    def test_usermod_update_password(self, mock_db, mock_admin):
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = {"username": "testuser"}  # User exists
        mock_db.return_value.cursor.return_value = mock_cursor

        args = MagicMock(username="testuser", passw="newpassword")
        cli.usermod(args)
